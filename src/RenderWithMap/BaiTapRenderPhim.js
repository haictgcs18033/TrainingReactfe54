...
import style from './BaiTapRenderPhim.module.css'
export default class BaiTapRenderPhim extends Component {
    ...
    renderPhim = () => {
        const arrMovie = this.mangPhim.map((movie, index) => {
            // const objectStyle = {
            //     width: 'auto',
            //     height: '200px'
            // }
            return <div className="col-4 mt-3" key={index}>
                <div className="card text-left">
                    {/* css theo tung component */}
                    <img className={`card-img-top w-100  ${style.hinhAnhPhim}`} src={movie.hinhAnh} alt />
                    {/* Inline style css cach 1 */}
                    {/* <img style={objectStyle} className="card-img-top w-100 " src={movie.hinhAnh} alt /> */}
                    {/*  Inline style css cach 2 */}
                    {/* <img style={{width: 'auto',height: '200px'}} className="card-img-top w-100 " src={movie.hinhAnh} alt /> */}
                    <div className="card-body bg-warning">
        <h4 className="card-title" style={{height:"50px"}}>{movie.tenPhim.length>50? <span>{movie.tenPhim.substr(0,50)}...</span>
        // ham substr(kitubatdau,kituketthuc)
        // ham substr co tac dung cat chuoi ki tu (nhu vi du tren ham se duyet tu 0 den 50 ki tu neu vuot qua ham se dung lai )
        :<span>{movie.tenPhim}</span>}</h4>
                        <p className="card-text">{movie.moTa.length>50 ? <span>{movie.moTa.substr(0,50)}...</span>
                        :<span>{movie.moTa}</span>}</p>
                    </div>
                </div>
